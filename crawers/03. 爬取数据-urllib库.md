...
### 3. Request对象
 其实上面的urlopen参数可以传入一个request请求,它其实就是一个Requesr,Data等等的内容。比如上面的两行代码，我们可以这么改写
...
>    举个例子：在豆瓣电影排行榜中，滚轮一直往下滑，发现url没变，但是内容在变多，滑动条在变短，这就是Ajax在偷偷的给服务器发请求呢，这时候要拿到Ajax的真实的url,需要打开开发者工具，network—>选择XHR(这就是Ajax的请求)

### 8. 请求 SSL证书验证（https = http+ssl）
